...
class ImageSubscriber(Node):
  ...
  def __init__(self):
    ...
    cv.resizeWindow("capture image", 960, 544)
    self.subscription
  ...
  def listener_callback(self, msg):
    ...
    if keyValue == 13:
      ...
      if self.initialize == False:
        self.image = captureImage.copy()
        self.initialize = True
      # 注册鼠标回调
      cv.setMouseCallback("capture image", self.mouse_callback, captureImage)
      cv.imshow("capture image", captureImage)
